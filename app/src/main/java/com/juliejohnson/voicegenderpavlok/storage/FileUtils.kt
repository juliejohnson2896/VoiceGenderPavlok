...
object FileUtils {
    ...
    /**
     * Loads a model file from the assets folder into a ByteBuffer.
     * This version reads the file as a raw byte stream to handle compressed assets.
     */
    fun loadOnnxModelFile(context: Context, filename: String): ByteBuffer {
        return try {
            ...
            // This is required by both TFLite and ONNX Runtimes.
            val byteBuffer = ByteBuffer.allocateDirect(bytes.size)
            byteBuffer.order(ByteOrder.nativeOrder())
            byteBuffer.put(bytes)
            byteBuffer.rewind() // Go back to the beginning of the buffer
            byteBuffer
